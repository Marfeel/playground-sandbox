...
describe("paywall experience", function () {
  let config, fixture;
  ...
  it("card should render on scroll", async function () {
    ...
    const firstCard = await browser.$(config.cards.paywall.cardSelector);
  ...
  // 	await dragCardBy(browser, config.cards.paywall.cardSelector, 200)
  ...
  it("activate card by click", async () => {
    ...
    const isAtActiveSnapPoint = await isAtSnapPoint(
      browser,
      config.cards.paywall.cardSelector,
      config.cards.paywall.snapPoints.active
    );
