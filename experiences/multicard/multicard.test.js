...
describe("multicard experience", function () {
  let config, fixture;
  ...
  it("card should render on scroll", async function () {
    ...
    const firstCard = await browser.$(config.cards.multicard.cardSelector);
  ...
  it("card should have right content", async function () {
    const rightContentLoaded = await isCardContentLoaded(
      browser,
      config.cards.multicard.cardSelector,
      config.cards.multicard.content
    );
  ...
  it("card should be displayed in viewport at initial snap point", async () => {
    ...
    const isAtInitialSnapPoint = await isAtSnapPoint(
      browser,
      config.cards.multicard.cardSelector,
      config.cards.multicard.snapPoints.initial
    );
  ...
  // 	await dragCardBy(browser, config.cards.multicard.cardSelector, 200)

  // 	const isAtMinimisedSnapPoint = await isAtSnapPoint(browser,
  // 		config.cards.multicard.cardSelector,
  // 		config.cards.multicard.snapPoints.minimised)

  // 	expect(isAtMinimisedSnapPoint).equal(true);
  // });

  it("activate card by click", async () => {
    await touchCard(browser, config.cards.multicard.cardSelector);

    const isAtActiveSnapPoint = await isAtSnapPoint(
      browser,
      config.cards.multicard.cardSelector,
      config.cards.multicard.snapPoints.active
    );
